...
app.put("/api/tables/:id/open", requireRole("ADMIN", "GERENTE", "CAIXA", "GARCOM"), async (req: AuthedRequest, res, next) => {
  try { const body = z.object({ customerName: optionalText }).parse(req.body); const table = await prisma.serviceTable.findUnique({ where: { id: asString(req.params.id) } }); if (!table) return res.status(404).json({ message: "Mesa nao encontrada." }); if (table.status !== "LIVRE") return res.status(400).json({ message: "Mesa ja esta ocupada." }); const updated = await prisma.serviceTable.update({ where: { id: table.id }, data: { status: "OCUPADA", waiterName: req.user?.name, customerName: body.customerName || null } }); await prisma.tableLog.create({ data: { tableId: table.id, userId: req.user?.id, action: "ABERTURA", description: `Mesa ${table.name} aberta por ${req.user?.name}${body.customerName ? ` - Cliente: ${body.customerName}` : ""}` } }); res.json(updated); } catch (error) { next(error); }
});
