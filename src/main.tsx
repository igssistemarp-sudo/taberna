...
function App() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const [me, dashboard, company, tables, customers, neighborhoods, categories, products, additions, paymentMethods, printers, orders, cash] = await Promise.all([
      ...
      setData({ user: me, dashboard, company, tables, customers, neighborhoods, categories, products, additions, paymentMethods, printers, orders, cash });
      if (company) {
        setCompanyDraft({
  ...
  if (!token) {
    return <div className="login-shell"><form className="login-card" onSubmit={doLogin}><div className="logo-mark">TB</div><h1>IGS Lanchonete PRO</h1><p>Taberna Food Beer</p><label>Login<input value={login} onChange={(e) => setLogin(e.target.value)} /></label><label>Senha<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></label>{error && <div className="alert">{error}</div>}<button disabled={loading}><LogIn size={16} /> Entrar</button></form></div>;
  }
...
function DashboardView({ data, money }: { data: AppData | null; money: (value: number) => string }) {
  ...
  const metrics = [
    ...
    { label: "Delivery em andamento", value: String(dashboard?.deliveryActive ?? 0), Icon: Truck },
...
function ReportsView({ token }: { token: string | null }) {
  const reports = ["sales_by_day", "sales_by_product", "sales_by_category", "sales_by_waiter", "sales_by_payment", "delivery_by_neighborhood", "delivery_fees", "top_products", "top_additions", "cancelled_orders", "cancelled_items", "profit", "payables", "receivables", "cashbook", "cash_closing", "financial_overall"];
  return <section className="panel"><h3>Relatorios</h3><div className="report-grid">{reports.map((report) => <article className="report-card" key={report}><strong>{report.replaceAll("_", " ")}</strong><div className="row-actions wrap"><button className="ghost" disabled={!token} onClick={() => token && void downloadReport(token, report, "csv")}>CSV</button><button className="ghost" disabled={!token} onClick={() => token && void downloadReport(token, report, "xlsx")}>Excel</button><button className="ghost" disabled={!token} onClick={() => token && void downloadReport(token, report, "pdf")}>PDF</button></div></article>)}</div></section>;
}
...
function SettingsView({ data, money, companyDraft, setCompanyDraft, mutate }: { data: AppData | null; money: (value: number) => string; companyDraft: { razaoSocial: string; nomeFantasia: string; onlineMenuSlug: string; serviceFeeEnabled: boolean; serviceFeePercent: number; openingHours: string; printerKitchenIp: string; printerBarIp: string; printerCashIp: string; printerPort: number; theme: string }; setCompanyDraft: React.Dispatch<React.SetStateAction<{ razaoSocial: string; nomeFantasia: string; onlineMenuSlug: string; serviceFeeEnabled: boolean; serviceFeePercent: number; openingHours: string; printerKitchenIp: string; printerBarIp: string; printerCashIp: string; printerPort: number; theme: string }>>; mutate: (path: string, options?: RequestInit) => Promise<void>; }) {
  return <div className="panel-grid"><section className="panel"><h3>Empresa</h3><div className="grid-2"><label>Razao social<input value={companyDraft.razaoSocial} onChange={(e) => setCompanyDraft((state) => ({ ...state, razaoSocial: e.target.value }))} /></label><label>Nome fantasia<input value={companyDraft.nomeFantasia} onChange={(e) => setCompanyDraft((state) => ({ ...state, nomeFantasia: e.target.value }))} /></label><label>Link cardapio<input value={companyDraft.onlineMenuSlug} onChange={(e) => setCompanyDraft((state) => ({ ...state, onlineMenuSlug: e.target.value }))} /></label><label>Horario<input value={companyDraft.openingHours} onChange={(e) => setCompanyDraft((state) => ({ ...state, openingHours: e.target.value }))} /></label><label>IP cozinha<input value={companyDraft.printerKitchenIp} onChange={(e) => setCompanyDraft((state) => ({ ...state, printerKitchenIp: e.target.value }))} /></label><label>IP bar<input value={companyDraft.printerBarIp} onChange={(e) => setCompanyDraft((state) => ({ ...state, printerBarIp: e.target.value }))} /></label><label>IP caixa<input value={companyDraft.printerCashIp} onChange={(e) => setCompanyDraft((state) => ({ ...state, printerCashIp: e.target.value }))} /></label><label>Porta<input type="number" value={companyDraft.printerPort} onChange={(e) => setCompanyDraft((state) => ({ ...state, printerPort: Number(e.target.value) }))} /></label><label>Taxa de servico<input type="number" value={companyDraft.serviceFeePercent} onChange={(e) => setCompanyDraft((state) => ({ ...state, serviceFeePercent: Number(e.target.value) }))} /></label></div><label><input type="checkbox" checked={companyDraft.serviceFeeEnabled} onChange={(e) => setCompanyDraft((state) => ({ ...state, serviceFeeEnabled: e.target.checked }))} /> Habilitar taxa de servico</label><button onClick={() => mutate("/api/company", { method: "PUT", body: JSON.stringify(companyDraft) })}>Salvar configuracoes</button></section><section className="panel"><h3>Impressoras</h3><div className="table-list">{data?.printers.map((item) => <div className="list-row" key={item.id}><strong>{item.name}</strong><span>{item.type}</span><span>{item.ip}:{item.port}</span><button className="ghost" onClick={() => mutate(`/api/printers/${item.id}/test`, { method: "POST" })}>Testar</button></div>)}</div></section></div>;
}
